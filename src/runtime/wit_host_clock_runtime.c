...
static void croft_wit_host_clock_reply_zero(SapWitHostClockClockReply* reply)
{
    if (!reply) {
        return;
    }
...
static void croft_wit_host_clock_reply_now_ok(SapWitHostClockClockReply* reply, uint64_t now_ms)
{
                                              ...
                                              SapWitHostClockClockReply* reply_out)
{
    ...
    switch (command->case_tag) {
        case SAP_WIT_HOST_CLOCK_CLOCK_COMMAND_MONOTONIC_NOW:
