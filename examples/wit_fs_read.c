...
static int expect_file_ok(const SapWitHostFsFsReply* reply, SapWitHostFsFileResource* handle_out)
{
    if (!reply || !handle_out) {
        return 0;
    }
...
static int expect_status_ok(const SapWitHostFsFsReply* reply)
{
    return reply
