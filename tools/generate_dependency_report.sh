...
categories=(
    ...
    "posix-filesystem|#include <(unistd|fcntl|dirent|sys/stat|sys/types)\\.h>|\\b(open|openat|close|read|write|pread|pwrite|lseek|dup|pipe|stat|fstat|fstatat|mkdir|mkdirat|rmdir|unlink|unlinkat|rename|renameat|symlink|symlinkat|readlink|readlinkat|link|linkat|opendir|fdopendir|readdir|closedir|getcwd|ftruncate|fsync|fdatasync|futimens|utimensat|access)\\s*\\("
    "posix-process-env|\\benviron\\b|\\b(getenv|setenv|unsetenv)\\s*\\("
    "posix-terminal|\\b(isatty)\\s*\\("
