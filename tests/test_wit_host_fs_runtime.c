...
static int wit_host_fs_expect_file_ok(const SapWitFsReply* reply, SapWitFileResource* handle_out)
{
    if (!reply || !handle_out) {
        return 0;
    }
...
static int wit_host_fs_expect_status_ok(const SapWitFsReply* reply)
{
    return reply
