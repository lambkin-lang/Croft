...
static int wit_host_fs_expect_file_ok(const SapWitHostFsFsReply* reply, SapWitHostFsFileResource* handle_out)
{
    if (!reply || !handle_out) {
        return 0;
    }
    if (reply->case_tag != SAP_WIT_HOST_FS_FS_REPLY_FILE
...
static int wit_host_fs_expect_status_ok(const SapWitHostFsFsReply* reply)
{
    return reply
...
int test_wit_host_fs_runtime_read_fixture(void)
{
    ...
    SapWitHostFsFsCommand command = {0};
    SapWitHostFsFsReply reply = {0};
    ...
    command.case_tag = SAP_WIT_HOST_FS_FS_COMMAND_OPEN;
    ...
    command.case_tag = SAP_WIT_HOST_FS_FS_COMMAND_READ_ALL;
    ...
    command.case_tag = SAP_WIT_HOST_FS_FS_COMMAND_CLOSE;
