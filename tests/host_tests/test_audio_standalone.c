...
int main(void) {
    ...
    struct { float f; float d; } melody[] = {
        ...
        { Fs4, 0.25f }, { A4, 0.25f }, { D4, 0.5f }, { 0.0f, 0.1f }
    };
    
    int num_notes = sizeof(melody) / sizeof(melody[0]);
    float pitch_sharp = 1.00289f; // +5 cents
    float pitch_flat = 0.99711f;  // -5 cents
    float tempo_multiplier = 0.75f; // Increase speed by reducing duration
    
    for (int i = 0; i < num_notes; i++) {
        float actual_duration = melody[i].d * tempo_multiplier;
        if (melody[i].f == 0.0f) {
            // Rest
            ...
            host_audio_play_tone(0.0f, 0.0f, actual_duration);
        } else {
            host_audio_play_tone(melody[i].f * pitch_sharp, melody[i].f * pitch_flat, actual_duration);
        }
    }
